...
In the image below, the plots show the neurons' activations per class. The blue hexagons size is proportional to the number of samples that activated each neuron. While looking at these plots is important to keep in mind that the blue hexagons size are normalized by the number of activation in each plot and not by the number of activation in the hole set of plots. <br/>
